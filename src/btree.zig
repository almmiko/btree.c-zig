...
pub fn Btree(comptime T: type, comptime Context: type) type {
    return struct {
        ...
        pub fn ascend(
            ...
            context: ?*ContextType,
            pivot: ?*const anyopaque,
            comptime iter: fn (a: *T, context: ?*ContextType) bool,
        ) bool {
            const cb = struct {
        ...
        pub fn iterator(self: *Self) Iterator {
            return .{ .btree_handle = self.handle };
        }

        pub const Iterator = struct {
            btree_handle: *c.struct_btree = undefined,
            handle: *c.struct_btree_iter = undefined,

            pub fn init(self: *const Iterator) Iterator {
                return .{
                    .handle = c.btree_iter_new(self.btree_handle).?,
                };
            }
            ...
            pub fn first(self: *const Iterator) bool {
            ...
            pub fn next(self: *const Iterator) bool {
                return c.btree_iter_next(self.handle);
            }

            pub fn prev(self: *const Iterator) bool {
                return c.btree_iter_prev(self.handle);
            }

            pub fn seek(self: *const Iterator, key: *const T) bool {
                return c.btree_iter_seek(self.handle, key);
            }

            pub fn item(self: *const Iterator) ?*T {
...
test "iterator" {
    ...
    const iter = btree.iterator().init();

    try std.testing.expectEqual(iter.seek(&user1), true);
    try std.testing.expectEqual(iter.item().?.id, 1);
}
